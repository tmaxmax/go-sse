...
// Writer returns a utility which can be used to add data to the Message
// using utilities that work with io.Writers.
//
// Each call to Writer returns a distinct MessageWriter instance, and state isn't
// shared between them. Do not do this:
//
//	m.Writer().WriteString("hello")
//	m.Writer().WriteString("world")
//	m.Writer().Close()
//
// as nothing will be written. Do this instead:
//
//	w := m.Writer()
//	w.WriteString("hello")
//	w.WriteString("world")
...
//	data: helloworld
//
// MessageWriter.Close must be called when you're done using it,
// as it will ensure that any data which didn't end with a newline will be
// added to the Message, and that memory used by the Writer is GCed.
// Writes are added when they end in a newline or Close is called. For example:
//
//	w := m.Writer()
//	w.WriteString("hello\n")
//	w.WriteString("world")
...
//	data: hello
//
// If "world" would be "world\n" or Close would be called, the output would be,
// as expected:
//
//	data: hello
//	data: world
//
// Read the documentation of MessageWriter for more detailed info.
func (e *Message) Writer() *MessageWriter {
	return &MessageWriter{m: e}
}
...
// A MessageWriter is returned by Message.Writer.
// It is used to write data to a Message in a stream-like manner
// and allows seamless integration with utilities that normally work only on io.Writers.
// You cannot use MessageWriter to stream data to clients. The MessageWriter adds data
// to the message as if it was added using AppendData. Do not use the MessageWriter
// after the Message was published!
//
// Data is added to Message in chunks – that is, when a newline sequence is encountered,
// all the data that's been written up to that newline will be added to the Message.
// If the Write data doesn't have a newline, it will be added to the Message only when
// a subsequent Write will contain a newline, the writes up to that point being buffered.
// You can avoid internal buffering by using WriteString with strings that end with a newline.
// To also add buffered data to the message, call Flush.
//
// MessageWriter implements:
//
//	io.Writer
//	io.StringWriter
//	io.ByteWriter
...
type MessageWriter struct {
	m *Message
	b []byte // could profile some programs to see if a sync.Pool is needed.
}

func (m *MessageWriter) Write(b []byte) (int, error) {
	s, c, hasNewline := string(b), "", false

	for s != "" {
		c, s, hasNewline = parser.NextChunk(s)
		m.b = append(m.b, c...)
		if !hasNewline {
			break
		}

		m.m.chunks = append(m.m.chunks, chunk{content: string(m.b)})
		m.b = m.b[:0]
	}
...
func (m *MessageWriter) WriteString(s string) (int, error) {
	c, hasNewline := "", false
	...
	for s != "" {
		...
		m.m.chunks = append(m.m.chunks, chunk{content: c})
	}

	return len(s), nil
}

func (m *MessageWriter) WriteByte(b byte) error {
	if b == '\r' || b == '\n' {
		m.m.chunks = append(m.m.chunks, chunk{content: string(m.b)})
		m.b = m.b[:0]
	} else {
...
// Close adds any buffered data that's left to the Message and cleans up resources.
// It never returns an error.
//
// Read the MessageWriter documentation for more info.
